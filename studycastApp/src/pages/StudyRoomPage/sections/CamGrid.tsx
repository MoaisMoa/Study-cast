...
export function CamGrid(props: CamGridProps) {
  const { members, elapsed, totalSec, timerSec, timerState, cam, camError = false, focusedId, setFocusedId,
    onTimerStart, onTimerPause, onTimerResume, onTimerReset } = props;
  const videoSelfRef = useRef<HTMLVideoElement>(null);

  // mock 캠: 실제 WebRTC 대신 본인 카메라만 getUserMedia 로 미리보기 (실패해도 아바타 폴백)
  useEffect(() => {
    let stream: MediaStream | null = null;
    if (cam && !camError && videoSelfRef.current) {
      navigator.mediaDevices?.getUserMedia({ video: true, audio: false })
        .then((s) => { stream = s; if (videoSelfRef.current) videoSelfRef.current.srcObject = s; })
        .catch(() => { /* 권한 거부/장치 없음 → 아바타 폴백 (camError로 상위에서 처리) */ });
    }
  ...
  const CamCell = ({ m, isSelf, camOn, avSize = 56, showTimer = false }: { m: RoomMember; isSelf: boolean; camOn: boolean; avSize?: number; showTimer?: boolean }) => (
    <div style={{
      ...
      {isSelf && camOn && !camError
        ? <video ref={videoSelfRef} autoPlay muted playsInline style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", transform: "scaleX(-1)" }} />
      ...
      {isSelf && showTimer && (
        <div style={{ position: "absolute", top: 8, right: 8, display: "flex", alignItems: "center", gap: 6, zIndex: 2 }}>
          ...
          {timerState === "idle" && <button onClick={(e) => { e.stopPropagation(); onTimerStart(); }} style={{ display: "flex", alignItems: "center", gap: 4, background: cam ? "#2e7d32" : "#9e9e9e", color: "#fff", border: "none", borderRadius: 6, padding: "4px 10px", cursor: cam ? "pointer" : "not-allowed", fontSize: 12, fontWeight: 600 }}><PlayIc s={12} />시작</button>}
  ...
  if (focusedId) {
    ...
    const mainIsSelf = main.role === "HOST";
    const mainCamOn = mainIsSelf ? cam : main.cam;
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, padding: 8, gap: 6 }}>
        <div style={{ flex: 1, minHeight: 0 }} onClick={() => setFocusedId(null)}>
          <CamCell m={main} isSelf={mainIsSelf} camOn={mainCamOn} avSize={90} showTimer={mainIsSelf} />
        </div>
        {thumbs.length > 0 && (
            ...
            {thumbs.map((m) => {
              ...
              return (
                <div key={m.id} style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => setFocusedId(m.id)}>
                  <CamCell m={m} isSelf={isSelf} camOn={isSelf ? cam : m.cam} avSize={36} />
                </div>
              );
            })}
          </div>
      ...
      {all.map((m, idx) => {
        ...
        return (
          <div key={m.id} style={{ gridColumn: count === 3 && isLast ? "1 / span 2" : undefined, minHeight: 0, height: "100%" }} onClick={() => setFocusedId(m.id)}>
            <CamCell m={m} isSelf={isSelf} camOn={isSelf ? cam : m.cam} avSize={count === 1 ? 90 : count === 2 ? 70 : 56} showTimer={isSelf} />
          </div>
        );
      })}
    </div>
