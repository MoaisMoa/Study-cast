...
<br>
<a src="https://study-cast-ten.vercel.app/"/>
...
- [트러블슈팅](docs/troubleshooting) — 개발 중 겪은 문제와 해결 과정

## 로컬 실행

**사전 준비**: PostgreSQL 실행 후 `studycast_db` 데이터베이스 생성 (`studycast/SQL/DDL.sql` 참고)

**백엔드**
```bash
cd studycast
# DB_PASSWORD, MAIL_USERNAME/PASSWORD, LIVEKIT_*, KAKAO_*, GOOGLE_*, TOKEN_ENCRYPT_KEY 등
# studycast/src/main/resources/application-secret.properties 에 설정 후
./gradlew bootRun
```
기본 프로필은 `local`이며 `http://localhost:8080`에서 실행됩니다.
...
npm run dev
```
`http://localhost:5173`에서 실행되며, 기본적으로 `http://localhost:8080` 백엔드를 바라봅니다.
